...
def read_data(train_filename='train.txt', test_filename='test.txt'):
    """
    extract the train and test information from the files
    :param train_filename: the name of the training data file
    :param test_filename: the name of the test data file
    :return: the train and test data organized in two ways: listed by example number, or listed by attributes.
    (the train and test are returned in both dimensions: n_attribute X n_examples and n_examples X n_attribute)
    """
...
def knn(train_set, test_set, k=5):
    """
    the prediction for each test example x, is the majority classification of the k nearest neighbors to x
    :param train_set: all the train set, ordered by examples and not by attributes
    :param test_set: all the test set, ordered by examples
...
def test_decision_tree(tree, test, attributes, att_poss_vals):
    ...
    :param tree: the decision tree that was based on the train data
    :param test: the test set, ordered by examples
    ...
    """
    # the test set is given as a list of all test examples, where each list is made up of a list of features
    correct = 0
    pred = []
...
def naive_bayes(data, attributes, att_classes):
    ...
    :param data: the train set, ordered by attributes
...
def naive_bayes_test(test_examples, attributes, att_classes, priors, probs_dictionary):
    ...
    :param test_examples: the test set ordered by examples
    :param attributes: list of attributes
...
def print_output(pred_dt, pred_knn, pred_nb, acc_dt, acc_knn, acc_nb):
    """
    print the outputs of the three models
    :param pred_dt: the prediction from the decision tree algorithm
    :param pred_knn: predictions from the k nearest neighbors algorithms
    :param pred_nb: predictions from the naive bayes algorithm
    :param acc_dt: accuracy for decision tree
    :param acc_knn: accuracy for knn
    :param acc_nb: accuracy for naive bayes
    :return: write the result to the output.txt file
    """
    out_file = open('output.txt', 'w')
    out_file.write('Num\tDT\tKNN\tnaiveBayes\n')
    for i in range(len(pred_nb)):
        out_file.write(str(i)+'\t'+pred_dt[i]+'\t'+pred_knn[i]+'\t'+pred_nb[i]+'\n')
    out_file.write('\t'+acc_dt+'\t'+acc_knn+'\t'+acc_nb)
    out_file.close()
...
def print_tree(dt):
    ...
    """
    out_file = open('output_tree.txt', 'w')
...
def main():
    ...
    pred_knn, acc_knn = knn(data_examples, test_examples)
    dt = make_decision_tree(data_attributes, attributes, att_classes_names, Counter(data_attributes[-1]).most_common(1)[0][0])
    pred_dt, acc_dt = test_decision_tree(dt, test_examples, attributes, att_classes_names)
    priors, probs_dictionaries = naive_bayes(data_attributes, attributes, att_classes_names)
    pred_nb, acc_nb = naive_bayes_test(test_examples, attributes, att_classes_names, priors, probs_dictionaries)
    print_output(pred_dt, pred_knn, pred_nb, acc_dt, acc_knn, acc_nb)
    print_tree(dt)
